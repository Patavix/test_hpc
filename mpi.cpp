...
void compare_edge(int *array, int size, int id, int world_size, bool &swap);
...
int main(int argc, char *argv[]) {
    ...
    int world_size;                            /* total number of tasks in partition */
    MPI_Comm_size(MPI_COMM_WORLD, &world_size); /* how many processes */
    
    MPI_Get_processor_name(hostname, &len);

    int size = atoi(argv[1]);
    int pad = (world_size - size%world_size)%world_size;
    int p_size = (size+pad)/world_size;
    ...
    MPI_Scatter(arr, p_size, MPI_INT, p_arr, p_size, MPI_INT, 0, MPI_COMM_WORLD);
    ...
    compare_edge(p_arr, p_size, rank, world_size, swapped);
    ...
    /* Add up the numbers */
    MPI_Gather(p_arr, p_size, MPI_INT, arr, p_size, MPI_INT, 0, MPI_COMM_WORLD);
...
void compare_edge(int *array, int size, int id, int world_size, bool &swapped) {
    MPI_Status status;
    int min = array[0];
